...
class GameOverSubstateEjected extends MusicBeatSubstate
{
	var bf:Boyfriend;
	...
	public function new(x:Float, y:Float)
	{
		...
		{
			if(!inSecret)
				FlxG.sound.play(Paths.sound('loss-defeat', 'shared'));
			else
				FlxG.sound.play(Paths.sound('no-balls', 'shared'));
		}
		else if(PlayState.SONG.player2 == 'bf-fall') {
			FlxG.sound.play(Paths.sound('loss_ejected', 'shared'));
		}
		else{
	...
	override function update(elapsed:Float)
	{
		...
		{
			if(PlayState.SONG.player2 != 'black')
				FlxG.sound.playMusic(Paths.music('gameOverAmogos' + stageSuffix));
			else
				FlxG.sound.playMusic(Paths.music('gameOverAmogosEmpty' + stageSuffix));
	...
	function endBullshit():Void
	{
		if (!isEnding)
		{
			...
			FlxG.sound.music.stop();
			if(PlayState.SONG.player2 != 'black')
				FlxG.sound.play(Paths.music('gameOverAmogosEnd' + stageSuffix));
			else
				FlxG.sound.play(Paths.music('gameOverAmogosEndEmpty' + stageSuffix));
